...
type LN() =
    ...
    [<Test>]
    [<Category "G2G_ReestablishRetransmit_Fundee">]
    member __.``reestablish is correct when node has to retransmit messages (fundee)``() = Async.RunSynchronously <| async {
        let! serverWallet, channelId = AcceptChannelFromGeewalletFunder()
        
        let channelFileName = serverWallet.ChannelStore.ChannelFileName channelId false
        let history = ResizeArray<SerializedChannel>()
        // watch for changes in channel file and load new version of serialized channel upon each save
        use watcher = new FileSystemWatcher(serverWallet.ChannelStore.ChannelDir.FullName)
        watcher.Filter <- Path.GetFileName channelFileName
        watcher.Changed.Add(
            fun _eventArgs -> history.Add(serverWallet.ChannelStore.LoadChannel channelId))
        ...
        let history = 
            ...
            |> Seq.filter (
                fun channel -> 
                    match channel.RemoteNextCommitInfo with
                    | Some(DotNetLightning.Channel.RemoteNextCommitInfo.Waiting _) -> true
                    | _ -> false)
        
        let serializedChannel = history |> Seq.last
        serverWallet.ChannelStore.SaveChannel serializedChannel
        Console.WriteLine("*** accepting reestablish...")
        let! acceptReestablishRes  = 
            ActiveChannel.AcceptReestablish serverWallet.ChannelStore serverWallet.NodeServer.TransportListener channelId
        match acceptReestablishRes with
        | Ok _ -> Console.WriteLine("*** accepted reestablish:")
        | Error err -> Assert.Fail(sprintf "Expected Ok, got Error: %A" err)

        (serverWallet :> IDisposable).Dispose()
    }
    
    [<Test>]
    [<Category "G2G_ReestablishRetransmit_Funder">]
    member __.``reestablish is correct when node has to retransmit messages (funder)``() = Async.RunSynchronously <| async {
        let! channelId, clientWallet, bitcoind, electrumServer, lnd, _fundingAmount = 
            OpenChannelWithFundee (Some Config.FundeeNodeEndpoint)
        
        let channelFileName = clientWallet.ChannelStore.ChannelFileName channelId false
        let history = ResizeArray<SerializedChannel>()
        // watch for changes in channel file and load new version of serialized channel upon each save
        use watcher = new FileSystemWatcher(clientWallet.ChannelStore.ChannelDir.FullName)
        watcher.Filter <- Path.GetFileName channelFileName
        watcher.Changed.Add(
            fun _eventArgs -> history.Add(clientWallet.ChannelStore.LoadChannel channelId))
        watcher.EnableRaisingEvents <- true

        let! sendResult = SendHtlcPaymentToGW channelId clientWallet "invoice.txt"
        UnwrapResult sendResult "Sending HTLC failed"
        
        let serializedChannelAfterPayment = clientWallet.ChannelStore.LoadChannel channelId
        ...
        let channelState = 
            history 
            |> Seq.filter (
                fun channel -> channel.SavedChannelState.LocalCommit.Index = CommitmentNumber.FirstCommitment.NextCommitment() )
            |> Seq.last
        clientWallet.ChannelStore.SaveChannel (channelState)
        
        try
            let! reestablishResult = clientWallet.NodeClient.ConnectReestablish channelId
            match reestablishResult with
            | Ok _ -> 
                Console.WriteLine("*** Reestablish successful")
            | Error err ->
                Assert.Fail("Error in reestablish: " + err.ToString())
            let channelAfterReestablish = clientWallet.ChannelStore.LoadChannel(channelId)
            Assert.AreEqual(channelAfterReestablish.Balance(), serializedChannelAfterPayment.Balance()) // ?
            Assert.That(
                channelAfterReestablish.SavedChannelState.RemoteCurrentPerCommitmentPoint.IsNone, 
                "Should not store my_current_per_commitment_point")
        with
        | exn ->
            Assert.Fail <| exn.Message
